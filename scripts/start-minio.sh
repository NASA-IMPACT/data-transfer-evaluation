...
# using input yaml file configuration from YAML_FILE env variable path.
# Usage:
...
src_username=`cat $YAML_FILE | grep -i source_token | cut -d':' -f 2 `
src_username=`echo $src_username | sed -e 's/^"//' -e 's/"$//'`

src_password=`cat $YAML_FILE | grep -i source_secret | cut -d':' -f 2 `
src_password=`echo $src_password| sed -e 's/^"//' -e 's/"$//'`

dest_username=`cat $YAML_FILE | grep -i dest_token | cut -d':' -f 2 `
dest_username=`echo $dest_username| sed -e 's/^"//' -e 's/"$//'`

dest_password=`cat $YAML_FILE | grep -i dest_secret | cut -d':' -f 2 `
dest_password=`echo $dest_password | sed -e 's/^"//' -e 's/"$//'`


src_port=`cat $YAML_FILE | grep -i source_s3_endpoint | cut -d':' -f 4 `
src_port=`echo $src_port| sed -e 's/^"//' -e 's/"$//'`

dest_port=`cat $YAML_FILE | grep -i dest_s3_endpoint | cut -d':' -f 4 `
